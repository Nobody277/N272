...
document.addEventListener('DOMContentLoaded', function () {
  const isReturningFromIndex = document.referrer.includes('index.html');
  initAnimatedBackground(isReturningFromIndex);
  initSchoolDashboard();
 ...
 * @param {boolean} isReturningFromIndex - Indicates if the user is returning from the index page.
 */
function initAnimatedBackground(isReturningFromIndex) {
  createStars(isReturningFromIndex);
  setInterval(createShootingStar, 2000);
}

/**
 * Creates a starfield background with optional twinkling.
 * @param {boolean} isReturningFromIndex - Indicates if initial opacity should be modified.
 */
function createStars(isReturningFromIndex) {
  ...
  for (let i = 0; i < starCount; i++) {
    ...
    const initialOpacity = isReturningFromIndex ? 0 : 0.1;
  ...
  const fadeDelay = isReturningFromIndex ? 300 : 100;
  const fadeDuration = isReturningFromIndex ? '1.5s' : '0.8s';
  setTimeout(() => {
    const stars = document.querySelectorAll('.star');
    stars.forEach(star => {
      const originalOpacity = star.getAttribute('data-original-opacity');
      star.style.transition = `opacity ${fadeDuration} ease`;
...
function setupBackButton() {
  ...
  if (backButton) {
    backButton.addEventListener('click', function () {
      ...
      setTimeout(function () {
        window.location.href = 'index.html?from=school';
      }, 800);
    });
  }
}
